...
export class Gameboard extends React.Component {
  ...
  displayPlayerInfo() {
	if (this.props.player_id != 0 ) {
      ...
      let cy = this.state.locations[this.props.character_id].currentY
      let roomName = this.state.grid[cx][cy].roomName;
	  let upic="/gameboard/"+uniqueIDs[this.props.character_id].image.replace('C','')+".png";
	  return (
		  <p>
			Username = {window.location.port} | Character = <img src={upic} style={{height:"20px",width:"15px"}}/> {uniqueIDs[this.props.character_id].name} | Location = {roomName}
		  </p>
	  );
	}
	else{
